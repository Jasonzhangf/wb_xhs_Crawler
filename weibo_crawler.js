...
async function processWeiboKeywordTask(task, weiboFS) {
    try {
        ...
        let history = { urls: [], folders: [] };
        if (fs.existsSync(historyFile)) {
            history = JSON.parse(fs.readFileSync(historyFile, 'utf8'));
            
            // 清理无效历史记录
            history.folders = history.folders.filter(folder => {
                return fs.existsSync(path.join(taskDir, folder));
            });
            
            history.urls = history.urls.filter(url => {
                return history.folders.some(folder => {
                    const contentFile = path.join(taskDir, folder, 'content.json');
            ...
            // Check if all items in history are recent enough to skip the task
            const recentThreshold = 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds
            const now = new Date();
            const allRecent = history.urls.every(urlItem => {
                const contentFile = path.join(taskDir, urlItem.folderPath, 'content.json');
                if (fs.existsSync(contentFile)) {
                    const content = JSON.parse(fs.readFileSync(contentFile, 'utf8'));
                    if (content.publishTime) {
                        const publishDate = new Date(content.publishTime);
                        return (now - publishDate) < recentThreshold;
                    }
                }
                return false;
            });
            
            if (allRecent && history.urls.length >= weiboConfig.MAX_ITEMS) {
                console.log('任务已存在且所有历史记录都是最近的，跳过任务');
                return 0;
            }
        }
        ...
        while (processedCount < weiboConfig.MAX_ITEMS && hasNextPage) {
            ...
            for (const feed of feedElements) {
                ...
                if (feed.postUrl && history.some(item => item.url === feed.postUrl)) {
                    ...
                    // Check if we've processed enough unique items
                    const uniqueProcessed = history.urls.filter(urlItem => 
                        !feedElements.some(f => f.postUrl === urlItem.url)
                    ).length;
                    
                    if (uniqueProcessed >= weiboConfig.MAX_ITEMS) {
                        console.log('已达到最大唯一项目数，终止任务');
...
// 处理微博用户任务
async function processWeiboUserTask(task, weiboFS) {
    try {
        // 检查任务文件夹是否存在
        const userId = task.user_id || "未知用户";
        const now = new Date();
        const dateStr = `${now.getFullYear()}${(now.getMonth()+1).toString().padStart(2, '0')}${now.getDate().toString().padStart(2, '0')}`;
        const maxItems = task.max_items || 20;
        const taskFolderName = `user_${userId}_${dateStr}_${maxItems}`;
        const taskDir = path.join(__dirname, 'data', 'weibo', taskFolderName);
    ...
    try {
        ...
        for (const task of tasks) {
            if (task.type === 'wb_user') {
                ...
                console.log('Verifying Chrome executable path...');
                const chromePath = weiboBrowser.getChromePath();
                console.log(`Chrome executable path verified: ${chromePath}`);
                
                // Set random user agent
                const userAgent = weiboBrowser.getRandomUserAgent();
                console.log(`Using random user agent: ${userAgent}`);
                
                // Initialize wait time
                console.log(`Initialization wait: ${weiboConfig.INIT_WAIT}ms`);
                await weiboBrowser.wait(weiboConfig.INIT_WAIT);
                
                // Process keyword task with proper logging
                console.log(`Preparing to process keyword task: ${task.keyword}`);
